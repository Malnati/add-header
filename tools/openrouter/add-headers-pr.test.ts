...
describe('loadIgnore with repository defaults', () => {
  test('mirrors default ignore patterns when repository file is present', () => {
    const dir = mkdtempSync(join(tmpdir(), 'add-header-default-ignore-'));
    try {
      writeFileSync(
        join(dir, '.addheaderignore'),
        ['node_modules/', '**/*.png', '**/*.lock'].join('\n') + '\n'
      );
      const ignores = loadIgnore(dir);
      assert.equal(ignores('node_modules/pkg/index.ts'), true);
      assert.equal(ignores('images/icon.png'), true);
      assert.equal(ignores('package.lock'), true);
